...
- [ ] Malformed DNI/RUC rejected before any source lookup.
- [ ] Doris OK + empty rows returns `[]` (no fixture) in prod tenants; fixture fires only on Exception.
- [ ] prestamype identifies against real Doris; non-existent DNI is rejected.
- [ ] prestaunion demo behavior unchanged (mock + fixture).
- [ ] Image builds and loads pymysql; full test suite green (baseline 424).
