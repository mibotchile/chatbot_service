# Spec: Widget Secure Distribution

> Parallel change: per-tenant-landings (server-side tenant resolution). Coordinate `tenant.config.json` schema: `publishable_key` + `embed_origins` fields must not conflict on `_template` merge.
...
Tests for the widget-key-gate, widget-distribution, and embed-cors capabilities MUST cover:

| Test | Covers |
|------|--------|
| `test_widget_gate.py` | unit: resolve_tenant_by_pk (single/dual/legacy), require_publishable_key factory |
| `test_widget_gate_integration.py` | gated routes (403 missing/unknown/bad-origin), allow_no_key bootstrap, CSRF composition |
| `test_cors_wildcard_guard.py` | `collect_embed_origins` rejects `"*"`, legitimate origins unaffected |
| `test_embed_widget.py` | versioned URL 200+immutable, unknown version 404, legacy /widget.js redirect, minified asset |
| `test_pk_loop.py` | /branding returns publishable_key, window.__PK__ injection, X-Publishable-Key sent on widget API calls |
