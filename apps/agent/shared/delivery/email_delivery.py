...
class EmailService:
    ...
    def __init__(self, api_url: str = "", from_email: str = "PrestaUnion <no-reply@prestaunion.pe>"):
    ...
    async def _send(
        ...
        if not self._enabled:
            logger.info(
                ...
                event, to_email, subject, [a.get("filename") for a in attachments],
            )
...
def _document_html(customer_name: str, doc_label: str, summary_html: str = "") -> str:
    ...
    attach_note = (
        "<p>Adjuntamos el documento solicitado en formato PDF.</p>"
        if not summary_html else ""
    )
    return f"""\
<!DOCTYPE html>
<html>
